...
#define PIN_SPI_DIN  46
#define PIN_SPI_CS   10
#define PIN_SPI_BUSY 12
#define PIN_SPI_RST  2
#define PIN_SPI_DC   11

#define PIN_SPI_CS_M    10
#define PIN_SPI_CS_S    3
#define PIN_SPI_PWR     1
...
void EPD_load_5in79b()
{
    Buff__bufInd -= 8;
    int pos = Buff__bufInd - Buff__getWord(Buff__bufInd);
    if (pos < 0) return;

    UDOUBLE Width = (EPD_5in79b_WIDTH % 16 == 0) ? (EPD_5in79b_WIDTH / 16) : (EPD_5in79b_WIDTH / 16 + 1);
    UDOUBLE Width1 = (EPD_5in79b_WIDTH % 8 == 0) ? (EPD_5in79b_WIDTH / 8) : (EPD_5in79b_WIDTH / 8 + 1);
    UDOUBLE Height = EPD_5in79b_HEIGHT;

    EPD_SendCommand(0x24);
    for (UDOUBLE j = 0; j < Height; j++) {
        for (UDOUBLE i = 0; i < Width / 2; i++) {
            if (pos >= Buff__bufInd) break;
            int value = Buff__getByte(pos);
            EPD_SendData((byte)value);
            pos += 2; 
        }
...
EPD_dispInfo EPD_dispMass[] =
{
    ...
    { EPD_13in3E_init,      EPD_load_13in3E6, -1,   EPD_load_13in3E6, EPD_13in3E_Show , "13.3 inch E"   },// 50
     { EPD_5in79B_init,      EPD_load_5in79b, 0x26, EPD_load_5in79b_red, EPD_5in79B_Show, "5.79 inch b" } // 51
};
