...
static void audio_apply_gain(int16_t *buf, size_t samples, float gain)
{
    if (!buf || samples == 0 || gain <= 0.0f)
        return;
...
int BoxAudioCodec::Write(const int16_t* data, int samples) {
    if (output_enabled_) {
        ...
        if (samples > 4096) return samples;

        memcpy(temp_buf, data, samples * sizeof(int16_t));

        float gain = 3.0f;  
        audio_apply_gain(temp_buf, samples, gain);

        ESP_ERROR_CHECK_WITHOUT_ABORT(esp_codec_dev_write(output_dev_, temp_buf, samples * sizeof(int16_t)));
    }
    return samples;
}