...
// audio gain
void audio_apply_gain(int16_t *buf, size_t samples, float gain)
{
    if (buf == NULL || samples == 0 || gain <= 0.0f)
        return;

    for (size_t i = 0; i < samples; i++)
    {
        int32_t sample = (int32_t)buf[i] * gain;
...
esp_err_t audio_write_with_gain(void *audio_buffer, size_t len, size_t *bytes_written, uint32_t timeout_ms)
{
    float gain = 3.0f; 
    ...
    audio_apply_gain(buf, samples, gain);
...
void page_audio_int(void)
{
    ...
    config.write_fn = audio_write_with_gain;
...
void app_main(void)
{
    ...
    else
    {
        ...
        //INT mute
        esp_codec_dev_set_out_mute(play_dev_handle, false);
        esp_codec_dev_set_out_vol(play_dev_handle, 80);
